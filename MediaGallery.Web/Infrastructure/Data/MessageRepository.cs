...
OFFSET @Offset ROWS FETCH NEXT @Limit ROWS ONLY;";
    ...
    private const string MediaChronologyQuery = @"SELECT m.ChannelID,
       m.MessageID,
       m.UserID,
       n.Username,
       n.FirstName,
...
LEFT JOIN dbo.Photos AS p ON p.PhotoID = m.PhotoID
LEFT JOIN dbo.Videos AS v ON v.VideoID = m.VideoID
WHERE (@PhotoId IS NOT NULL AND m.PhotoID = @PhotoId)
   OR (@VideoId IS NOT NULL AND m.VideoID = @VideoId)
ORDER BY m.SentDate ASC, m.MessageID ASC;";
    ...
    public async Task<IReadOnlyList<MessageDetailDto>> GetMediaChronologyAsync(
        long? photoId,
        long? videoId,
        CancellationToken cancellationToken = default)
    {
        using var connection = CreateConnection();
        using var command = new SqlCommand(MediaChronologyQuery, connection)
        {
            CommandType = CommandType.Text
        };

        command.Parameters.Add(new SqlParameter("@PhotoId", SqlDbType.BigInt)
        {
            Value = photoId.HasValue ? photoId.Value : DBNull.Value
        });
        command.Parameters.Add(new SqlParameter("@VideoId", SqlDbType.BigInt)
        {
            Value = videoId.HasValue ? videoId.Value : DBNull.Value
        });
